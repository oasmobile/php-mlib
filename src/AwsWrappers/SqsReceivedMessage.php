...
use Oasis\Mlib\Data\ArrayDataProvider;
use Oasis\Mlib\Data\InvalidDataTypeException;
use Oasis\Mlib\Exceptions\Runtime\InvalidResponseDataException;
    ...
    public function validate()
    {
        if ($this->md5OfBody) {
            if ($this->md5OfBody != md5($this->body)) {
                throw new InvalidResponseDataException("Body md5 doesn't match!");
            }
        }
        ...
        if ($this->md5OfAttributes) {
            ...
            foreach ($attributes as $k => $v) {
                ...
                if (MUtils::stringStartsWith($v['DataType'], "Binary")) {
                    $transport_type = 2;
                }
                else {
                    $transport_type = 1;
                ...
                switch ($v['DataType']) {
                    ...
                    case "Binary":
                        ...
                        break;
                    default:
                        throw new InvalidDataTypeException("Unknown data type: {$v['DataType']}");
                        break;
                }
            ...
            if ($this->md5OfAttributes != md5($encoded_data)) {
                throw new InvalidResponseDataException("Attribute md5 doesn't match!");
            }
        }
    }
